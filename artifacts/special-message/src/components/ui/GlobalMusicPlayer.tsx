...
function Player() {
  ...
  const playlist = [
    { url: '/audio/lagu1.mp3', title: 'Best Part', artist: 'H.E.R. ft. Daniel Caesar', cover: '/audio/cover1.jpg' },
    { url: '/audio/lagu2.mp3', title: 'Double Take', artist: 'Dhruv', cover: '/audio/cover2.jpg' },
    { url: '/audio/lagu3.mp3', title: 'P.S. I Love You', artist: 'Paul Partohap', cover: '/audio/cover3.jpg' },
    { url: '/audio/lagu4.mp3', title: 'Photograph', artist: 'Ed Sheeran', cover: '/audio/cover4.jpg' },
    { url: '/audio/lagu5.mp3', title: 'Old Love', artist: 'Yuji ft Putri Dahlia', cover: '/audio/cover5.jpg' },
  ];
  ...
  const wrapperStyle = {
    ...
    minWidth: visible ? 320 : 220,
    padding: visible ? 20 : '12px 14px',
    borderRadius: visible ? 22 : 999,
    background: visible ? 'linear-gradient(180deg,#ffd8df,#ffe3e9)' : 'linear-gradient(180deg,#ffd6de,#ffe9ef)',
    boxShadow: visible ? '0 24px 60px rgba(176,75,86,0.14)' : '0 16px 36px rgba(176,75,86,0.12)',
      ...
      {visible ? (
        <>
          ...
          <div style={{width: 124, height: 124, borderRadius: 16, background: 'linear-gradient(180deg,#fff,#ffeef2)', boxShadow: '0 12px 30px rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
            <img src={currentTrack.cover} alt="art" style={{width: 104, height: 104, borderRadius: 12, objectFit: 'cover'}} />
          </div>
          ...
          <div style={{display: 'flex', width: '100%', alignItems: 'center', gap: 12}}>
            <div style={{width: 30, height: 30, borderRadius: 10, background: 'rgba(255,255,255,0.88)', display: 'grid', placeItems: 'center', flexShrink: 0}}>
        ...
        <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', width:'100%', gap: 8}}>
          <div style={{display:'flex', alignItems:'center', gap: 8, minWidth: 0, flex: 1}}>
            <img src={currentTrack.cover} alt="cover" style={{width: 38, height: 38, borderRadius: 12, objectFit: 'cover', flexShrink: 0}} />
            <div style={{display:'flex', flexDirection:'column', gap: 2, minWidth: 0}}>
              <span style={{fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{currentTrack.title}</span>
              <span style={{fontSize: 10, color: '#8a2f33', opacity: 0.75, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'}}>{currentTrack.artist}</span>
            </div>
          </div>
          <div style={{display:'flex', alignItems:'center', gap: 6, flexShrink: 0}}>
            <span style={{fontSize: 10, color: '#8a2f33', opacity: 0.8, minWidth: 34, textAlign: 'right'}}>{formatTime(currentTime)}</span>
            <button aria-label="open player" onMouseEnter={() => setHoveredControl('open')} onMouseLeave={() => setHoveredControl(null)} onClick={(e) => { e.stopPropagation(); setVisible(true); }} style={{...iconBtnStyle(), width: 32, height: 32, borderRadius: 12, background: 'rgba(255,255,255,0.95)', boxShadow: '0 8px 18px rgba(0,0,0,0.08)', transition: 'all 0.2s ease', ...(hoveredControl === 'open' ? { transform: 'translateY(-2px)', boxShadow: '0 10px 20px rgba(176,75,86,0.16)' } : {})}}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6 10l6-6 6 6" stroke="#b04b56" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M6 14l6 6 6-6" stroke="#b04b56" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            </button>
          </div>
        </div>
      )}
